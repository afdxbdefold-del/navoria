...
/**
 * Entfernt AUSSCHLIESSLICH die Fachrichtungs-Beschreibung aus einem Praxisnamen.
 * Titel wie "Dr. med.", "Prof.", etc. bleiben ERHALTEN – der Slug soll den exakten Namen tragen.
 *
 * Beispiele:
 *   "Dr. med. Jaroslaw Raczynski Facharzt für Allgemeinmedizin"
 *   → "Dr. med. Jaroslaw Raczynski"
 *
 *   "Barbara Piontek Fachärztin für Allgemeinmedizin"
 *   → "Barbara Piontek"
 *
 *   "Praxis am Braunenberg Dr. Thomas Bopp"
 *   → "Praxis am Braunenberg Dr. Thomas Bopp" (unverändert)
 */
function cleanNameForSlug(fullName) {
  if (!fullName) return '';
  return String(fullName)
    // Fachbezeichnungen abschneiden (mit oder ohne Komma davor)
    .replace(/,?\s*(Fachärztin?|Facharzt|Fachzahnärztin?|Fachzahnarzt|Ärztin|Arzt|Zahnärztin?|Zahnarzt|Praxisgemeinschaft|Gemeinschaftspraxis|MVZ|Ordination)\s+für\s+.*/i, '')
    .replace(/,?\s*(Fachärztin?|Facharzt|Fachzahnärztin?|Fachzahnarzt)\s*$/i, '')
    .trim();
}
