 ...
 */
export async function createCampaign({ cities, specialtySlugs, maxPerQuery = 60, filterNoWebsiteOnly = false, name }) {
  ...
  const campaign = {
    id,
    name: name || `Kampagne ${cities.length} Städte × ${specialties.length} Fachrichtungen${filterNoWebsiteOnly ? ' (nur ohne Website)' : ''}`,
    cities,
    specialty_slugs: specialtySlugs,
    max_per_query: maxPerQuery,
    filter_no_website_only: !!filterNoWebsiteOnly,
    queries,
    total_queries: queries.length,
    done_queries: 0,
    found: 0, inserted: 0, updated: 0, skipped: 0, skipped_discarded: 0, skipped_has_website: 0, errors: 0,
    current_query: null,
    status: 'running',
  ...
  runCampaignWorker(id, maxPerQuery, !!filterNoWebsiteOnly).catch(async (err) => {
...
export async function resumeCampaign(id) {
  ...
  runCampaignWorker(id, c.max_per_query || 60, !!c.filter_no_website_only).catch(async (err) => {
...
async function runCampaignWorker(campaignId, maxPerQuery, filterNoWebsiteOnly = false) {
  const col = await getCollection('campaigns');
  ...
  stats.skipped_discarded = campaign.skipped_discarded || 0;
  stats.skipped_has_website = campaign.skipped_has_website || 0;
  ...
  const saveProgress = async (extra = {}) => {
    await col.updateOne({ id: campaignId }, {
      $set: {
        ...
        skipped: stats.skipped, skipped_discarded: stats.skipped_discarded,
        skipped_has_website: stats.skipped_has_website, errors: stats.errors,
  ...
  const runOne = async (q) => {
    ...
    try {
      const r = await importOneQuery({ city: q.city, query: q.query, placeType: q.placeType, maxResults: maxPerQuery, filterNoWebsiteOnly });
      stats.found += r.found;
      stats.inserted += r.inserted;
      stats.updated += r.updated;
      stats.skipped += r.skipped;
      stats.skipped_discarded += r.skipped_discarded || 0;
      stats.skipped_has_website += r.skipped_has_website || 0;
