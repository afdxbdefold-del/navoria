 ...
 */
export async function backfillMissingFields({ limit = 50, minAgeHours = 24 } = {}) {
  const doctorsCol = await getCollection('doctor_places');
  const threshold = new Date(Date.now() - minAgeHours * 3600 * 1000);
  const targets = await doctorsCol.find({
    google_place_id: { $exists: true, $ne: null },
    $or: [
      { last_external_sync_at: { $exists: false } },
      { last_external_sync_at: null },
      { last_external_sync_at: { $lt: threshold } },
    ],
  }).limit(limit).toArray();
