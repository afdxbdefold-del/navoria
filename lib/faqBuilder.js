...
export function buildFaqs(d, { humanizedType, hasHours } = {}) {
  ...
  // 6. Website – Zwei Fälle:
  //    (a) Externe Website eingetragen  → normale Website-FAQ
  //    (b) Auf Navoria verifiziert ("abgehakt") ohne externe Website  → Navoria IST die offizielle Praxis-Seite
  //    (c) Weder noch → FAQ komplett ausgeblendet (kein "nicht bekannt"-Textbaustein)
  if (d.website_url) {
    items.push({
      q: `Gibt es eine Website von ${displayName}?`,
      a: `Ja, die auf dieser Seite hinterlegte Website ist ${d.website_url}.`,
    });
  } else if (d.is_verified && d.city_slug && d.slug) {
    const profileUrl = `https://navoria.de/praxis/${d.city_slug}/${d.slug}`;
    items.push({
      q: `Wo finde ich die offizielle Website von ${displayName}?`,
      a: `Die Praxis unterhält aktuell keine eigene Website. Ihre offizielle Praxis-Seite ist der verifizierte Navoria-Eintrag unter ${profileUrl}. Alle Angaben werden regelmäßig geprüft.`,
    });
  }
